...
---------------------------------------------------------
-- 1. MINIMAP BUTTON (LibDataBroker)
---------------------------------------------------------
-- Setup for the minimap icon and its tooltip
local LDB = LibStub("LibDataBroker-1.1"):NewDataObject(addonName, {
    ...
    OnTooltipShow = function(tooltip)
        ...
        -- 2. Title in Classcolor
        tooltip:AddLine("n1mmelUI", color.r, color.g, color.b)
...
function ns.InitDB()
    ...
    N1mmelUIDB.minimapIcon = N1mmelUIDB.minimapIcon or {
        hide = false
    }
    if N1mmelUIDB.crestFontSize == nil then
        N1mmelUIDB.crestFontSize = 11
    end
    if N1mmelUIDB.showCrestFrame == nil then
        N1mmelUIDB.showCrestFrame = false
    end
    if N1mmelUIDB.crestFramePos == nil then
        N1mmelUIDB.crestFramePos = {"CENTER", nil, "CENTER", 0, 0}
    end
    if N1mmelUIDB.minimapZoom == nil then
        N1mmelUIDB.minimapZoom = 0
    end
    if N1mmelUIDB.showMapCoords == nil then
        N1mmelUIDB.showMapCoords = true
    end
    if N1mmelUIDB.ilvlChar == nil then
        N1mmelUIDB.ilvlChar = true
    end
    if N1mmelUIDB.ilvlBags == nil then
        N1mmelUIDB.ilvlBags = true
    end
    if N1mmelUIDB.squareMinimap == nil then
        N1mmelUIDB.squareMinimap = false
    end
    if N1mmelUIDB.targetClassColor == nil then
        N1mmelUIDB.targetClassColor = false
    end
    if N1mmelUIDB.skipCinematics == nil then
        N1mmelUIDB.skipCinematics = false
    end
    if N1mmelUIDB.hideTalkingHead == nil then
        N1mmelUIDB.hideTalkingHead = false
    end
    if N1mmelUIDB.afkScreen == nil then
        N1mmelUIDB.afkScreen = false
    end
    if N1mmelUIDB.ilvlCharSize == nil then
        N1mmelUIDB.ilvlCharSize = 13
    end
    if N1mmelUIDB.ilvlBagsSize == nil then
        N1mmelUIDB.ilvlBagsSize = 13
    end
    if N1mmelUIDB.whisperAlert == nil then
        N1mmelUIDB.whisperAlert = false
    end
    if N1mmelUIDB.whisperSound == nil then
        N1mmelUIDB.whisperSound = "Bell"
    end
    if N1mmelUIDB.shortChannels == nil then
        N1mmelUIDB.shortChannels = true
    end
    if N1mmelUIDB.chatClassColors == nil then
        N1mmelUIDB.chatClassColors = true
    end
    if N1mmelUIDB.chatURLs == nil then
        N1mmelUIDB.chatURLs = true
    end
    if N1mmelUIDB.unitFrameFonts == nil then
        N1mmelUIDB.unitFrameFonts = true
    end
    if N1mmelUIDB.unitFrameFontSize == nil then
        N1mmelUIDB.unitFrameFontSize = 13
    end
...
---------------------------------------------------------
-- 4. BOOT SEQUENCE (Addon Loaded & Player Login)
---------------------------------------------------------
-- This is the main engine starting all modules
local eventFrame = CreateFrame("Frame")
eventFrame:RegisterEvent("ADDON_LOADED")
eventFrame:RegisterEvent("PLAYER_LOGIN") -- Wartet, bis der Charakter da ist!

eventFrame:SetScript("OnEvent", function(self, event, arg1)

    -- PHASE 1: Datenbank laden
    if event == "ADDON_LOADED" and arg1 == addonName then
        ns.InitDB()

        -- Minimap Icon registrieren
        icon:Register(addonName, LDB, N1mmelUIDB.minimapIcon)
        if N1mmelUIDB.minimapIcon.hide then
            icon:Hide(addonName)
        end

        -- PHASE 2: UI bauen (Spieler ist jetzt 100% geladen)
    elseif event == "PLAYER_LOGIN" then
        ...
        if ns.SetupChatModifications then
            ns.SetupChatModifications()
        end
